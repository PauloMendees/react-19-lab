...
const isAuthorized = (device: MediaDeviceInfo) => device.deviceId && device.label;

const getAuthorizedCameras = (devices: MediaDeviceInfo[]) => {
  return devices.filter((device) => device.kind === 'videoinput' && isAuthorized(device));
};

const getAuthorizedMicrophones = (devices: MediaDeviceInfo[]) => {
  return devices.filter((device) => device.kind === 'audioinput' && isAuthorized(device));
};
...
export const getDeviceId = (devices: MediaDeviceInfo[], preferredDeviceId: string) => {
